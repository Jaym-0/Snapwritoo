...
export default async function PitchSection() {
  ...
  if (average !== null) {
    ...
    const runsIndex = displayStats.findIndex((s) => s.label.toLowerCase() === "runs");
    const insertAt = runsIndex === -1 ? displayStats.length : runsIndex + 1;
    displayStats.splice(insertAt, 0, { id: "average", label: "Average", value: average });
  }
  ...
  return (
    <section id="pitch">
      <div className="section-head">
        <h2>On the pitch</h2>
        <p>A club-level batting record, kept the way a scorer keeps it — one line at a time.</p>
      </div>
